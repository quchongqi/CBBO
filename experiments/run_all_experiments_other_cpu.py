...
# ============================================================
def worker(task_queue, num_threads=8):
    import torch
    ...
    while True:
        ...
        try:
            # Block on the task queue with a timeout to prevent workers from
            # incorrectly assuming the queue is empty in multiprocessing settings.
            task = task_queue.get()
        except queue.Empty:
            # Terminate the worker only after the queue has been empty for a while
            break
