...
eta_ls = ['None', 0.001, 0.01, 0.1, 1, 10, 100, 1000]
...
for eta_l in eta_ls:
    for name, bounds in Bounds.items():
        ...
        if eta_l == 'None':
            plt.title(rf" $\ell_i = 1  \quad (L_1 = L_2 = ${bounds[1][0] - bounds[0][0]} )", fontsize=20)
        else:
            plt.title(rf" $\ell_i = {eta_l} \cdot L_i \quad (L_1 = L_2 = ${bounds[1][0] - bounds[0][0]} )", fontsize=20)
        plt.tight_layout()
        if eta_l == 'None':
            plt.savefig(f'figures/my_test_CBBO/2D_adaptive_kernel/fixed_kernel_{name}_{eta_l}.pdf')
        else:
            plt.savefig(f'figures/my_test_CBBO/2D_adaptive_kernel/adaptive_kernel_{name}_{eta_l}.pdf')
